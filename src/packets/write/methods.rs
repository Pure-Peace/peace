...
pub async fn user_stats(p: &Player) -> PacketData {
    PacketBuilder::with(id::BANCHO_USER_STATS)
        .add_multiple(&mut [
            write_num(p.id),
            write_num(p.game_status.action as u8),
            write_string(&p.game_status.info),
            write_string(&p.game_status.beatmap_md5),
            write_num(p.game_status.mods.value),
            write_num(p.game_status.mode.raw_value()),
            write_num(p.game_status.beatmap_id),
            write_num(p.stats.ranked_score),
            write_num(p.stats.accuracy / 100f32),
