...
pub fn user_stats(
    ...
    PacketBuilder::with(id::BANCHO_USER_STATS)
        .add_multiple(&mut [
            ...
            write_num(action),
            write_string(info),
            write_string(beatmap_md5),
            write_num(mods),
            write_num(mode),
            write_num(beatmap_id),
            write_num(ranked_score),
            write_num(accuracy / 100f32),
            write_num(playcount),
            write_num(total_score),
            write_num(rank),
            write_num(pp),
        ])
        .pack()
...
pub fn user_presence(
    ...
    PacketBuilder::with(id::BANCHO_USER_PRESENCE)
        .add_multiple(&mut [
            ...
            write_num(utc_offset + 24),
            write_num(country_code),
            write_num((bancho_priv | 0) as u8),
            write_num(longitude),
            write_num(latitude),
            write_num(rank),
        ])
